                    ...
                    <div class="row g-0">
                        ...
                        <div class="col-md-9">
                            <div class="card-body">
                                ...
                                <p><a href="modificar_ver/modificar_socio.php?id=' . $id . '" class="btn btn-warning">Modificar</a></p>
...
function listarNombreDeSociosTestimonios($conexionBD)
{

    $sentencia = "SELECT nombre FROM socio";
    $consulta = $conexionBD->prepare($sentencia);
...
function listarNombreDeSociosCitas($conexionBD)
{
...
function listarServicios($conexionBD)
{
    ...
    if ($resultado->num_rows > 0) {
        ...
        echo '<p class="text-center text-muted">No hay servicios registrados.</p>';
    }
}
...
function listarUltimasNoticias($conexionBD)
{
    ...
    $sentencia = "SELECT * FROM noticia WHERE fecha<=? ORDER BY fecha DESC LIMIT 3";
    $consulta = $conexionBD->prepare($sentencia);
...
function gestionCita($conexionBD, $id_socio, $id_servicio, $condicion)
{
    if ($condicion === "b") {
        ...
        // Si ya esta cancelada se borra la cita
...
function listarCitasPorNombreSocio($conexionBD, $nombreSocio)
{
    ...
    while ($consulta->fetch()) {
        ...
        $hayCitas = true;
        $boton = $cancelada
            ? '<a href="modificar_ver/gestion_cita.php?id_socio=' . $id_socio . '&id_servicio=' . $id_servicio . '&condicion=a" class="btn btn-danger btn-sm mt-2">Borrar Cita</a>'
            : '<a href="modificar_ver/gestion_cita.php?id_socio=' . $id_socio . '&id_servicio=' . $id_servicio . '&condicion=b" class="btn btn-warning btn-sm mt-2">Cancelar Cita</a>';
        ...
        <div class="collapse navbar-collapse" id="navbarNav">
            <ul class="navbar-nav ms-auto">
                <li class="nav-item mb-2 mb-md-2">
                    <a class="nav-link text-primary bg-white rounded px-3 py-2 mx-1" href="php/socios.php">Socios</a>
                </li>
                <li class="nav-item mb-2 mb-md-2">
                    <a class="nav-link text-primary bg-white rounded px-3 py-2 mx-1" href="php/servicios.php">Servicios</a>
                </li>
                <li class="nav-item mb-2 mb-md-2">
                    <a class="nav-link text-primary bg-white rounded px-3 py-2 mx-1" href="php/testimonios.php">Testimonios</a>
                </li>
                <li class="nav-item mb-2 mb-md-2">
                    <a class="nav-link text-primary bg-white rounded px-3 py-2 mx-1" href="php/noticias.php">Noticias</a>
                </li>
                <li class="nav-item mb-2 mb-md-2">
                    <a class="nav-link text-primary bg-white rounded px-3 py-2 mx-1" href="php/citas.php">Citas</a>
                </li>
            </ul>
        </div>
    </div>
